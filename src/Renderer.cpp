...
void Renderer::Render(const Model& model, const Mat4& view, const Mat4& proj)
{
	...
	for (int i = 0; i < nTris; i++) {
		DrawTexturedTriangleSSE(screenSpaceTris[i], model.texture);
	}
}
...
{
	constexpr int simdVectorSize = 4;
	constexpr int simdAlignment = simdVectorSize * sizeof(float);
	...
	if (minX < 0) minX = 0.5f;
	if (maxX > width - 1) maxX = width - (simdVectorSize + 0.5f); // ex: 1920x1080. 1920 - 4.5 = 1915.5. 1915.5+4=1919.5=max column we can access.
	if (minY < 0) minY = 0.5f;
	if (maxY > height - 1) maxY = height - 0.5f;

	// Align AABB for SIMD
	minX = ((int)minX / simdVectorSize) * simdVectorSize; // round down
	maxX = ((int)(maxX + 0.999f) / simdVectorSize) * simdVectorSize; // round up
	minX += 0.5f;
	maxX += 0.5f;
	...
	auto w0ColumnIncrement = _mm_set1_ps(simdVectorSize * (v1.y - v2.y));
	auto w1ColumnIncrement = _mm_set1_ps(simdVectorSize * (v2.y - v0.y));
	auto w2ColumnIncrement = _mm_set1_ps(simdVectorSize * (v0.y - v1.y));
